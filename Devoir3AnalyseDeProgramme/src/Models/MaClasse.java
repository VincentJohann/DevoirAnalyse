...
public class MaClasse 
{
	...
	public String findAttributs(String attributeName) 
	{
		String attributeClass="Classe de type inconnu";
		for(Parameter item : Attributs) 
		{
			if(item.getName().equals(attributeName))
				attributeClass=item.getType();    		
		}
	...
	public String getMethodesString() 
	{
		...
		for(Method myMethod : Methodes) {
			...
			if(myMethod.getInnerMethods().size()>0) {
				for(Method inmyMethod : myMethod.getInnerMethods()) 
				{
					allMethods += "Inner method   "+inmyMethod.getName() +"("+inmyMethod.getParameterString()+") \r\n";
				}
			}
